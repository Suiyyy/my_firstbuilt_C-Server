...
import static com.assetsms.util.FXMLPage.UPDATE_ASSETSUSING;
import static java.util.Objects.nonNull;

public class ListAssetsUsing extends BaseController implements Initializable {
    @FXML
    private TableView<AssetsTrjn> assetsUsingListTable;
    @FXML
    ...
    private void  doUpdate(ActionEvent event){
        ...
        if (nonNull(editAssetsTrjn )) {
            try {
                navigate(UPDATE_ASSETSUSING.getTitle(),(Node)event.getSource(), UPDATE_ASSETSUSING.getPage());
            } catch (IOException e) {
                //发生异常后通过警告对话框显示异常信息
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        //把assetsTrjnData中满足条件操作字段为领用的数据记录添加到列表中
        FilteredList<AssetsTrjn> filteredData = new FilteredList<>(assetsTrjnData, n -> n.getFromAcc().contains("领用"));
        //FilteredList<AssetsTrjn> filteredData = new FilteredList<>(assetsTrjnData, n -> true);//显示的是数据记录
        ...
        assetsUsingListTable.setItems(filteredData);
    }
    // 得到列表上选择的客户信息
    private AssetsTrjn getItem() {
        return AssetsTrjn.class.cast(assetsUsingListTable.getSelectionModel().getSelectedItem());
    }
}
