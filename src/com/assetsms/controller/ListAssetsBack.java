...
import static com.assetsms.util.FXMLPage.UPDATE_ASSETSBACK;
import static java.util.Objects.nonNull;

public class ListAssetsBack extends BaseController implements Initializable {
    @FXML
    private TableView<AssetsTrjn> assetsBackListTable;
    @FXML
    ...
    private void  doUpdate(ActionEvent event){
        ...
        if (nonNull(editAssetsTrjn )) {
            try {
                navigate(UPDATE_ASSETSBACK.getTitle(),(Node)event.getSource(), UPDATE_ASSETSBACK.getPage());
            } catch (IOException e) {
                //发生异常后通过警告对话框显示异常信息
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        FilteredList<AssetsTrjn> filteredData = new FilteredList<>(assetsTrjnData, n -> n.getFromAcc().contains("归还"));
        ...
        assetsBackListTable.setItems(filteredData);
    }
    // 得到列表上选择的客户信息
    private AssetsTrjn getItem() {
        return AssetsTrjn.class.cast(assetsBackListTable.getSelectionModel().getSelectedItem());
    }
}
